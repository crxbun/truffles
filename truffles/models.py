...
class User(db.Model):
    ...
    username = mapped_column(VARCHAR, unique=True, nullable=False)
...
class Truffle(db.Model):
    ...
    name = mapped_column(VARCHAR, unique=True, nullable=False)
...
class TruffleAccessories(db.Model):
    ...
    headwear = mapped_column(VARCHAR, nullable=True)

class Chatroom(db.Model):
    __tablename__ = 'chatroom'
    id = mapped_column(Integer, unique=True, primary_key=True)
    message = mapped_column(VARCHAR, nullable=False)
    userID = mapped_column(ForeignKey('user.id'), nullable=False)
